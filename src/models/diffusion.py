...
class GaussianDiffusion(nn.Module):
    ...
    def p_mean_variance(self, denoise_fn, x, t, clip_denoised: bool, denoise_kwargs={}, post_fn=identity):
        ...
        if clip_denoised:
            x_recon.clamp_(-4.0, 4.0)
            # mean = 0.06730790532348715
            # std = 0.13365173653612408
            # x_recon.clamp_((-std-mean)/std, (std-mean)/std)
    ...
    def focal_tversky_loss(self,predictions, targets, alpha=0.6, gamma=3/4, beta=0.4, smooth=1):

        #maggiore alpha maggiore è l'interesse per la classe 1, quindi di conseguenza maggiore è beta più favorisco la classe 0

        # Calcola la Tversky Loss
        intersection = th.sum(predictions * targets)
        false_positive = th.sum(predictions * (1 - targets))
        false_negative = th.sum((1 - predictions) * targets)
        
        tversky = (intersection + smooth) / (intersection + alpha * false_positive + beta * false_negative + smooth)
        
        # Calcola la Focal Loss
        focal_tversky = (1 - tversky) ** gamma * tversky

        # Riduci la perdita media su tutte le dimensioni
        loss = th.mean(focal_tversky)
    ...
    def p_losses(self, denoise_fn, x_0, noise=None, denoise_kwargs={}, post_fn=identity):
        ...
        #prendo numero di elementi nel batch
        b = x_0.size(0)
        dev = x_0.device

        #per ogni elemento del batch campiono un t
        ...
        #creo rumore di dimensione come il ground truth
        noise = default(noise, lambda: th.randn_like(x_0))
        #rumorizzo il ground truth creandone questa versione rumorosa chiamata x_noisy
        x_noisy = self.q_sample(x_0, continuous_sqrt_alpha_cumprod, noise)
        #do x_noisy alla Unet
        ...
        if self.model_mean_type == "eps":
            ...
            #loss = F.binary_cross_entropy(x_recon, x_0)
            ...
            #loss = self.focal_tversky_loss(predictions=x_recon, targets=x_0)
    ...
    def sample_ddim(
        ...
        for i, step in enumerate(pbar):
            ...
            if clip_denoised:
                pred_x0.clamp_(-4.0, 4.0)
                # mean = 0.06730790532348715
                # std = 0.13365173653612408
                # pred_x0.clamp_((-std-mean)/std, (std-mean)/std)
