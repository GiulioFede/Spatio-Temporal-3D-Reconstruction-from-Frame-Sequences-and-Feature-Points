...
class SR3dPreprocessor(trainer.BasePreprocessor):
    ...
    def __call__(self, batch, augmentation=False) -> dict:
        ...
        '''
            s.im_y:  (b,1,64,64,64) sdf fine
            s.im_x:  (b,1,64,64,64) sdf coarse
            s.im_x2: (b,1,24,64,64) frames
        '''
        s.synset, s.model_id, s.im_y, s.im_x, s.im_x2, s.c = batch
        #(b, 1, 64,64,64)
        s.im_y = s.im_y.to(self.device, non_blocking=True)
        #(b, 1, 32, 32, 32)
        s.im_x = s.im_x.to(self.device, non_blocking=True)
        
        s.im_x2 = s.im_x2.to(self.device, non_blocking=True)

        s.c = s.c.to(self.device, non_blocking=True)
        s.n = len(s.im_x)

        # blur_augmentation (NON LA FA)
        ...
        #Porta la 32x32x32 a 64x64x64 cosi da avere compatibilità con l'input (NB: SAREBBE L'UP SAMPLING CHE C'è NEL PAPER)
        s.im_x = F.interpolate(s.im_x, s.im_y.shape[2:])

        # flip augmentation (LA FA)
...
class SR3dTrainer(trainer.BaseTrainer):
    ...
    def step(self, s):
        '''
            x_t: versione rumorosa del ground truth (b,1,64,64,64)
            s.pim_x: condition sdf coarse (b,1,64,64,64)
            t: timestep
        '''
        def denoise_fn(x_t, t):
            if self.gaussian_conditional_augmentation:
                ...
                #prendo la condition 1
                z_t = s.pim_x
                z_t2 = s.pim_x2
            #preparo l'input per ls Unet: ground truth rumoroso (x_t) + condition (z_t)
            input = th.cat([x_t], dim=1)
            if self.gaussian_conditional_augmentation:
                out = self.model_optim(input,z_t,z_t2, t, c=s.c, s=t_z)
            else:
                #do l'input alla Unet (NB: passo anche la classe 'c' a cui ogni elemento del batch appartiene)
                #'out' è la predizione del ground truth senza noise
                out = self.model_optim(input,z_t,z_t2, t, c=s.c)
    ...
    def step_test(self, s):
        def denoise_fn_wrapper(model):
            def denoise_fn(x_t, t):
                ...
                input = th.cat([x_t], dim=1)
                if self.gaussian_conditional_augmentation:
                    out = model(input,z_t,z_t2, t, c=s.c, s=t_z)
                else:
                    out = model(input,z_t,z_t2, t, c=s.c)
                if self.predict_residual:
                    out = out + s.pim_x
                return out
        ...
        with tqdm(total=n, ncols=100, file=sys.stdout, desc="Sample", disable=not self.rankzero) as t:
            for c in self.class_idx_rank:
                ...
                for i in range(0, self.n_samples_per_class, b):
                    ...
                    self.step_test(s)
                    im = th.stack([th.nn.functional.interpolate(s.im_x, size=(self.image_size,self.image_size,self.image_size), mode='nearest'), s.im_y, s.im_p], dim=1).flatten(0, 1)
                    ims.append(im)

                    if self.ema_decay is not None and s.im_p_ema is not None:
                        im = th.stack([th.nn.functional.interpolate(s.im_x, size=(self.image_size,self.image_size,self.image_size), mode='nearest'), s.im_y, s.im_p_ema], dim=1).flatten(0, 1)
                        ims_ema.append(im)
