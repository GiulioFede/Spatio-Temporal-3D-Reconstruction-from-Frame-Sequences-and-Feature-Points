...
from src.models.vision_transformers import VisionTransformer
from src.models.frames_visual_transformers import FRAMES_VisionTransformer
...
class UNet(nn.Module):
    def __init__(
        ...
        # self.fuse = VisionTransformer(embed_dim=16*16*4, 
        #                     hidden_dim=16*16*4, 
        #                     num_channels=4, 
        #                     num_heads=8, 
        #                     num_layers=6, 
        ...
        #                     final_dimension=64,
        #                     what_to_return= "frames")

        #self.frames_encoder = FRAMES_VisionTransformer()

        #rete per ridurre i frame in 32x32x32
        # self.frames_conv1 = nn.Conv3d(in_channels=1, out_channels=32, kernel_size=(64,8,8), stride=(1,8,8))
        # self.frames_convtransposed = nn.ConvTranspose2d(in_channels=32, out_channels=32, kernel_size=(25,25), stride=(1, 1))

    '''
        x: concatenzazione tra ground truth rumoroso e coarse --> (b, 2, 64,64,64)
        cond: frames -> (b,1,24,64,64)
    
    '''

    def forward(self, x, cond_sdf, cond_frames, t, c=None, s=None):

        #sdf_frames_fused = self.fuse(cond_frames, cond_sdf)

        # with th.no_grad():
        #     frames_encoding = self.frames_encoder(cond_frames)

        # cond_frames = self.frames_conv1(cond_frames)
        # cond_frames = self.frames_convtransposed(cond_frames.squeeze(2)).unsqueeze(1)

        x = th.cat([cond_frames,cond_sdf, x], dim=1)
        ...
        #trasformo ogni pixel 3D in un valore tra 0 e 1 ad indicarmi la probabilità di appartenere alla classe 1
        #x = nn.Sigmoid()(x)
