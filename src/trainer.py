...
class BaseTrainer(BaseWorker):
    ...
    def save(self, out_path):
        data = {
            ...
            "last_lr": self.optim.param_groups[0]['lr']
        }
        torch.save(data, str(out_path))
        print("Corrente learning rate: ", self.optim.param_groups[0]['lr'] )

    def load(self):
        data = torch.load("/home/giuliofederico/SDF-Diffusion_x32_conditioned/results/new_dataset/240320_003250_towns/best_ep0912.pth", map_location='cpu')
        
        print("Carico ultimo modello:")
        #print(data)
        print("epoch:", data['epoch'])
        print("best loss", data['best_loss'])
        self.model.load_state_dict(data['model'], strict=False)
        self.best = data['best_loss']
        self.epoch = data['epoch']
        print("Last lr: ",data["last_lr"] )
    ...
    def train_epoch(self, dl: "DataLoader", prefix="Train"):
        ...
        if self.rankzero:
            #indica la corrente epoca sul totale (es. Train [0001/1000])
            desc = f"{prefix} [{self.epoch:04d}/{self.args.epochs:04d}]"
            t = tqdm(total=len(dl.dataset), ncols=150, file=sys.stdout, desc=desc, leave=True)
        for batch in dl:
            '''
                Batch è una lista di lunghezza 5:
                0: ...
                1: ...
                2: contiene le 1x64x64x64 sdf
                3: contiene le 1x32x32x32 sdf
                4: ...
            '''
